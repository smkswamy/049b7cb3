...
import { fetchActivities, unarchiveCall } from '../services/api';
...
const ArchivedCalls = () => {
  ...
  const handleUnarchive = async (activites) => {
    await Promise.all(activities.map(activity => unarchiveCall(activity.id)));
    const data = await fetchActivities();
    setActivities(data.filter(activity => activity.is_archived));
  };
  ...
  return (
    <div className={classes.root}>
      <Button
        ...
        onClick={() => handleUnarchive(activities)}
      >
        Unarchive all calls
      </Button>
